    ...
    <script>
        $(document).ready(function() {
                ...
                columns: [
                    {
                        ...
                        render: function(data, type, row) {
                            return `
                                <a href="{{ route('onduty.edit', '') }}/${row.id}" class="btn btn-link text-warning" title="Edit">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <a href="javascript:void(0);" onclick="delConfirm(${row.id})" class="btn btn-link text-danger" title="Hapus">
                                    <i class="fas fa-trash-alt"></i>
                                </a>
                                <a href="javascript:void(0);" onclick="togglePublish(${row.id})" class="btn btn-link text-primary" title="Toggle Publish">
            ...
            window.togglePublish = function(id) {
                swal({
                    title: 'Ubah Publish?',
                    text: 'Record ini akan diubah menjadi "published". Data lain di lokasi yang sama akan di-unpublish.',
                    icon: 'info',
                    buttons: true,
                    dangerMode: false
                }).then((willToggle) => {
                    if (willToggle) {
                        $.ajax({
                            ...
                            success: function(response) {
                                swal('Berhasil', 'Status publish berhasil diperbarui!', 'success')
                                    .then(() => table.ajax.reload());
                            },
                            error: function(xhr) {
                                let errorMsg = 'Terjadi kesalahan. Silakan coba lagi.';
